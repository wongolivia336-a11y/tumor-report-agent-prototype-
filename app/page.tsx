...
type InspectorTopic = "process" | "warnings" | "generation" | "review" | "artifacts";
type PreviewSection = "recognized" | "issues" | "qa" | "context";
type PreviewKind = "validation" | "review";
type ArtifactPreviewKind = "word" | "package" | "prism" | "figure" | "qc" | "evidence" | "review-doc";
type UserEvent = {
  id: string;
  after: "upload" | "warning" | "review";
  text: string;
};
type FollowupState = "idle" | "thinking" | "answered";

const validationActionsBase = [
  {
    label: "读取方案 DOCX",
...
const classifyFile = (file: File): UploadedFile["kind"] => {
...
const expertProfiles: Record<string, { role: string; finding: string }> = {
  "数据核对专家 D": {
    role: "报告与 Excel / 方案溯源核对",
    finding: "本轮未发现数据冲突或溯源断裂。",
  },
  "统计复核专家 C": {
    role: "公式、统计路径、p-value、结论措辞",
    finding: "建议 SD 确认 TV/TGI 显著措辞与 p-value 方法来源。",
  },
  "安全性专家 S": {
...
function ConfirmCard({
  ...
  return (
    <article className={`confirmCard ${tone}`}>
      <div>
        <span>{eyebrow}</span>
        <h3>{title}</h3>
        <p>{detail}</p>
